...
export function VqeProofPanel({
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setExecutions(parseVqeExecutions(await response.json()));
      setState("ready");
    } catch (cause) {
      setState("error");
      ...
      <section className="mj-studio-main">
        <div className="mj-studio-main-head">
          <div className="mj-studio-title-block">
            <span className="mj-section-label">Atlas VQE · Phase 5A</span>
            <h1>H₂ STO-3G actual-VQE proof</h1>
        ...
        <div className="mj-studio-empty" role="note">
          ...
          <p>
            {ja
              ? "人間レビュー未完了・production runtime未資格・公開実行禁止です。保存物もprivate候補のままです。"
              : "Human review is pending, the production runtime is unqualified, public execution is blocked, and materialized evidence remains private."}
          </p>
        </div>
        ...
        <div className="mj-studio-actions">
          <button className="mj-primary-button" type="button" disabled={busy !== null || Boolean(selected)} onClick={() => void mutate("start")}>
            {busy === "start" ? (ja ? "開始中…" : "Starting…") : (ja ? "ローカル候補を実行" : "Run local candidate")}
          </button>
