...
import { MIN_PROMPT_LENGTH, planText } from "../lib/workflow-planner/run-context.ts";
...
/**
 * `informsNala` is true where the composer sends the planner's reading with
 * the run (the signed-in Run page); the cue then says so, because the reader
 * should know what the model is given.
 */
export function NalaPlanCue({ prompt, locale, informsNala = false }: { prompt: string; locale: PublicLocale; informsNala?: boolean }) {
    ...
    <p className="mj-run-plan-cue" data-problem={problem.id}>
      {locale === "ja" ? (
        <>
          ...
          {informsNala ? "送信すると、Nala はこのワークフローと出典つきのコストをもとに計画します。" : null}
        </>
      ) : (
        <>
          ...
          the papers.
          {informsNala ? " When you send it, Nala plans with that workflow and its cited costs." : null}
        </>
      )}{" "}
