...
export async function register() {
  ...
  if (process.env.SENTRY_DSN) {
    Sentry.init({
      ...
      // releaseSha() (lib/deploy-env.ts) reads LEONA_GIT_COMMIT_SHA, set as
      // both a build arg and a runtime env var on Cloud Run — absent (and
      // harmless as undefined) in local dev/CI. Ties an event to the exact
      // commit it came from.
      release: releaseSha(),
    });
  }
