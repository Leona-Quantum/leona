...
// The ONE angle formatter. A second one lived here briefly and disagreed with
// this one: the Atlas figure drew `RY(pi/3)` where Studio's diagram drew
// `RY(π/3)` for the same step. See gate-param-label.ts.
...
export function stepLabel(step: BuilderStep, customGates: readonly CustomGateDefinition[]): string {
  ...
  return step.param ? `${step.gate}(${formatGateParam(step.param)})` : step.gate;
}
