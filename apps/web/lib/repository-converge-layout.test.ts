...
test("a line that opens into something says so, and a line that does not is not a link", () => {
  ...
  //
  // 88 → 89 and 27 → 28 in session 129: `berry-multistep` is one more top-level
  // route on `linear-ode-solve`, and it opens, because the two hops it delegates
  // are route segments rather than ingredients.
