...
test("a lane's ingredients are its own, not its descendants'", () => {
  ...
  // themselves hang ingredients matches every one of them by prefix: on the
  // `time-discretization` figure with backward Euler open, the prefix names
  // eleven ingredients for a lane that needs one, all ten extras belonging to the
  // linear solvers two levels down. The count is asserted non-zero rather than
  // pinned, because it is the *reason* the field exists — if it ever reaches
  // zero, that is not a fix, it is a graph that stopped nesting, and the
  // equivalence would then look safe to anyone who came back to simplify this.
  let lanes = 0;
  let owned = 0;
  let overReported = 0;
  for (const focus of drawableSlots(LAYER_GRAPH, STATE_VOCABULARY)) {
