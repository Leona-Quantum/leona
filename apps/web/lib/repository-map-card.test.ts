...
test("Theory is held on every method, and each hop inside it is empty or filled for the right reason", () => {
  ...
  assert.equal(hops, 130, `${hops} hops, not 130`);
...
test("a method's card says what it is a narrower version of, and what narrows it", () => {
  ...
  assert.equal(declared.length, 7, `${declared.length} methods declare refines, not 7`);
  ...
  assert.deepEqual(
    ...
    ["adapt-ansatz", "lchs-route", "sabre-routing", "taylor-all-at-once"],
    "the refinements section holds on exactly the four folded parents",
  );
});
...
test("the unnamed stretch is 56 of 63 methods, one each, and 13 of them follow a named step", () => {
  ...
  assert.equal(withOwn.length, 76);
