...
**回路構造。** 標準的な分解では、各量子ビット$j$（最上位から）にアダマールを適用し、続いて下位ビットを制御とする制御位相ゲート$R_k=\mathrm{diag}(1,e^{2\pi i/2^k})$を適用し、最後に量子ビットの順序を反転します（$\lfloor n/2\rfloor$回のSWAP、あるいは配線の relabeling で実装）。これによりちょうど$n$個のアダマールと$n(n-1)/2$個の制御位相ゲート、合計$O(n^2)$ゲートとなります。Coppersmithの近似QFTはカットオフ$m$より小さい角度$2\pi/2^m$の制御回転を省略し、周期発見や位相推定など多くの用途で指数的に小さい誤差のみでゲート数を$O(n\log n)$に削減します。
