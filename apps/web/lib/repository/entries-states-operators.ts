...
**開いた鎖との対比。** パスグラフのクラスター状態は端の量子ビットが1つしか隣接点を持たない（生成子$X_1Z_2$と$Z_{n-1}X_n$は重み2）のに対し、リングの生成子はすべて重み3です。余分な巻き付き辺が境界効果を取り除き、量子ビットの巡回的な relabeling に対してスタビライザー群を並進不変にします。
