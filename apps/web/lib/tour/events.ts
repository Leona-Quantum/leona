...
/**
 * Asks the workspace shell to open or close its navigation drawer. On a phone the rail
 * and the sidebar sit in a collapsed drawer, so a step pointing at one of them would
 * otherwise report its control as not on screen; and a step pointing at the page must
 * not be left under a drawer an earlier step opened. `detail.open === false` closes it,
 * anything else opens it. The shell acts only at phone width.
 */
export const WORKSPACE_SIDEBAR_EVENT = "leona:workspace-sidebar";
/**
 * Asks the workspace shell to open its account menu, for a step that points at an item
 * inside it (Usage, Settings) when the reader arrives there with the menu shut: a
 * resumed tour, a `#tour=` link, or Back from a step inside Settings.
 */
export const WORKSPACE_ACCOUNT_MENU_EVENT = "leona:workspace-account-menu";
