...
function vqeTransverseIsing(): WorkedExample {
  ...
  return {
    ...
    customGates: [],
    notes: [
      note(steps[0].id, "One of the four angles an optimiser would be free to turn. π/2 is not a textbook value: it came out of a deterministic coarse grid search over this file's own simulator (see the comment on VQE_ANGLES), which is how these four constants were fixed. A real VQE reaches angles like these by measuring the energy and adjusting; this example does not run that loop.", "最適化器が自由に回せる4つの角度のひとつです。π/2 は教科書的な値ではなく、このファイル自身のシミュレータ上での決定的な粗いグリッド探索から得られた値です（VQE_ANGLES のコメントを参照）。実際のVQEはエネルギーの測定と調整を繰り返してこのような角度に到達しますが、この例はそのループを実行していません。"),
      ...
      note(steps[3].id, "The second layer's rotation on qubit 0. The search settled on zero here, so it does nothing at all — the honest outcome, not a mistake: the layer has to be in the circuit for the search to have had the choice.", "2番目の層の、量子ビット0への回転です。探索はここでゼロに落ち着いたため、何も起こりません。誤りではなく正直な結果です。探索がその選択肢を持つためには、この層が回路に存在していなければなりません。"),
      note(steps[4].id, "And the last of the four, also zero. A two-layer ansatz turned out to need only its first layer for this Hamiltonian — something the search found, and not something you can see by inspecting the circuit.", "4つのうちの最後のひとつで、これもゼロです。2層のアンザッツは、このハミルトニアンに対しては結局最初の層しか必要としませんでした。これは回路を眺めてではなく、最適化を実行してはじめて分かることです。"),
    ],
...
function hiddenShift4(): WorkedExample {
  ...
  return {
    ...
    notes: [
      ...
      note(hAll2.step.id, "The second Hadamard layer. Only g has been applied so far, shifted and then unshifted; this transforms that into an intermediate phase pattern. The f oracle and a THIRD Hadamard layer still have to follow before anything is readable — it is the last of the three that puts the shift into the computational basis.", "2回目のアダマール層です。ここまでに適用されたのは、シフトして戻した g だけです。この層はそれを中間的な位相パターンに変換します。読み取れる形になるには、f のオラクルと3回目のアダマール層がまだ必要です。シフトを計算基底に出すのは、3つのうちの最後の層です。"),
