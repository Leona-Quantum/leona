...
export const WORKSPACE_COPY: Record<PublicLocale, {
  ...
  ja: {
    ...
    studio: {
      ...
      newDraft: "Vaultに保存する前の回路を試すための新しい下書きです。",
      copyCode: "コードをコピー",
      copied: "コピー済み",
      ...
      cpuUnavailable: (reason) => ({
        artifact_required: "この保存済み回路のシミュレーション記録を作成する前に、下書きをVaultへ保存してください。",
      ...
      simulationArtifactRequired: "この保存済み回路のシミュレーション記録を作成する前に、下書きをVaultへ保存してください。",
      ...
      simulationBoundary: "対応しているゲートをもとに、ブラウザ上で状態ベクトルシミュレーションを実行します。実行した下書きの識別情報は記録しますが、未保存の編集はVaultのバージョンを更新しません。この履歴はローカルシミュレーションの結果であり、正式な検証結果や実機の実行結果ではありません。",
      ...
      hardwareVerifiedRequired: "量子コンピュータで実行するには、この回路を検証してVaultに保存してください。",
      ...
      editorNote: "下書きを直接編集できます。Vaultに新しいバージョンとして保存するには、シミュレーションまたは検証を実行してください。",
      versionHistory: "バージョン履歴",
      repositoryView: "Atlasで見る",
      currentVersion: (id) => `現在 · ${id}`,
      draftNotSaved: "下書き · 未保存",
      currentVersionNote: "検証に合格した実行を新しいバージョンとして保存するまで、Vaultの現在のバージョンは変更されません。",
...
export const ACCOUNT_COPY: Record<PublicLocale, {
  ...
  ja: {
    title: "設定",
    lede: "プロフィール、Vault、ワークスペース、表示設定を管理します。",
    signOut: "サインアウト",
    preferences: "表示設定",
    language: "言語",
    ...
    autoKeepHelp:
      "既定ではオフです。オフのときは、実行後に保存するか確認します。結果は開いて変換・編集できますが、保存を選ぶまでVaultには追加されません。",
    autoKeepOn: "今後の実行結果は自動的にVaultへ保存されます。",
    autoKeepOff: "今後の実行結果は保存前に確認します。",
    autoKeepFailed: "設定を変更できませんでした。",
    workspaceBoundaries: "ワークスペースごとのデータ管理",
    library: "Vault",
    libraryHelp: "保存した実行結果や Atlas の公開資料は、個人用の Vault に保管されます。",
    repositoryExport: "Atlas から Vault に追加",
    repositoryExportHelp: "サインインすると、Atlas の公開資料をこのワークスペースの Vault に追加し、Studio で開けます。",
    collaboration: "共同作業",
    ...
    usageTitle: "使用状況と上限",
    usageHelp: "アーリーアクセス期間中は料金がかかりません。実行回数や Vault の保存容量を増やしたい場合はご相談ください。",
    usagePlan: "プラン",
    usagePlanValue: "アーリーアクセス",
    usageRuns: "実行",
    usageRunsValue: "フェアユース — アーリーアクセス中は固定上限なし",
    usageStorage: "Vault の保存容量",
