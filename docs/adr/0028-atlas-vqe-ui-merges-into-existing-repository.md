# ADR-0028: The Atlas VQE Registry/Compare UI merges into the existing `/repository` page, not a new top-level section
...
code, by design (Phase 2/ADR-0027 explicitly stopped short of execution;
...
4. Every `unknown`/`conflicting`/`null` value already present in the corpus
   ...
   an omitted row (plan requirement, restated from ADR-0027: never display
