...
| WORKOS_CLIENT_ID / WORKOS_API_KEY / WORKOS_COOKIE_PASSWORD | apps/web/.env.local (dev) + GCP Secret Manager (deploy — `WEB_WORKOS_API_KEY`/`WEB_WORKOS_COOKIE_PASSWORD`, mounted by `deploy-web.yml`; `WORKOS_CLIENT_ID` is a plain, non-secret env var in that same workflow) | AuthKit (web); the API needs WORKOS_CLIENT_ID only. Until Vercel was retired (2026-09-21, ADR-0033) the web values lived in Vercel env instead; there is no preview environment any more to hold placeholders in | 2026-07-10 |
| SENTRY_DSN + NEXT_PUBLIC_SENTRY_DSN | api/worker: GCP Secret Manager (`SENTRY_DSN`), mounted by `deploy.yml` via `--update-secrets`. web: GCP Secret Manager (`WEB_SENTRY_DSN`, mounted by `deploy-web.yml`; until 2026-09-21 this was Vercel env, Production only) | api+worker share one DSN (Sentry project `python`, no per-service split — see `majorana_service` tag instead), web has its own (project `web`), org `majorana-ms`. DSNs are client keys, not strict secrets, but stored properly rather than as a literal now that both are wired | 2026-07-11, wired 2026-08-15 (ai-ops#97) |
...
| OTEL_EXPORTER_OTLP_ENDPOINT / OTEL_EXPORTER_OTLP_HEADERS | local secrets store (dev); GCP Secret Manager at deploy (pending) | all services → Grafana Cloud OTLP gateway (stack `yellowwildebeest692`); the header embeds token `majorana-otlp` (otlp-write scope) — treat as secret. Still not wired into `deploy-web.yml`'s env/secrets composition as of this sweep, same "pending" state as before the Vercel retirement | 2026-07-11 |
...
| Vercel | account `eshmis` (team `majoranaq`) | project `web` was linked from apps/web 2026-07-10; its Git integration was disconnected 2026-09-21 (ADR-0033) and it builds nothing now. The team itself is not retired — the code-execution sandbox (ADR-0006) is a separate Vercel project and stays live; see `packages/py/sandbox` and `docs/runbooks/sandbox-image.md`, not this row |
...
| Sentry | org `majorana-ms`, projects `python` (api+worker) + `web` | US region, free dev tier; errors only (AD-10). MAJORANA_ENV=production and MAJORANA_RELEASE (git SHA) are set by `deploy.yml` (api/worker) and `deploy-web.yml` (web; until 2026-09-21 the latter was Vercel); `python` events also carry a `majorana_service` tag (`api`/`worker`) |
