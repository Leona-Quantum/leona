...
> **Split provenance (2026-08-11).** This record documents runs made on the original
> #365 branch. The changes landed on `dev` through the split PRs (#370 migration,
> #379 reaper, #380 agent, #384 verification, #385 intent, and the harness PR carrying
> this file), with the review findings fixed in the split — so figures below describe
> the original branch, not `dev` as merged. The provider/model line under
> "Verification commands" is the operator's recorded configuration; the committed
> report artifacts do not themselves carry provider or model fields.
