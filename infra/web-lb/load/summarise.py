...
def main(path, out=None):
    ...
    with open(path) as f:
        for line in f:
            ...
            if row["metric"] == "resp":
                counts[(tags.get("scenario"), tags.get("cls"), tags.get("code"), tags.get("src"))] += int(data["value"])
    ...
    for key in sorted(by):
        ...
        print(f"{key[0]:8} {key[1]:7} n={n:6}  429={n429:6} ({100 * n429 / n:5.1f}%)  " +
              ", ".join(f"{k}:{v}" for k, v in c.most_common()))
    for sc, v in sorted(lat.items()):
        print(f"latency {sc:8} p50={pct(v, 50):.0f}ms p95={pct(v, 95):.0f}ms p99={pct(v, 99):.0f}ms max={max(v):.0f}ms")
    if out:
        json.dump({"window": [first, last],
                   "counts": [{"scenario": k[0], "cls": k[1], "code": k[2], "src": k[3], "n": n}
                              for k, n in sorted(counts.items(), key=lambda x: tuple(map(str, x[0])))],
                   "latency_ms": {sc: {"p50": pct(v, 50), "p95": pct(v, 95), "p99": pct(v, 99), "max": max(v)}
