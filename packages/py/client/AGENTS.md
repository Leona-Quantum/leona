...
| `client.py` | `Client`: the bearer-token control-plane calls (notebooks, runs, estimates) generalised from `leona_notebooks.jupyter.Client`, plus the Atlas convenience methods that wrap `atlas.py`/`catalog.py`. |
