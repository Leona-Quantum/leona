...
function titleFor(summary: VerificationSummary): string {
  ...
  if (summary.reason_code === "ai_review_aligned") return "Executed and AI-reviewed";
...
export function VerificationSummaryPanel({
  ...
  const warning = advisoryOutcome
    ? "The generated code ran, its basic result contract passed, and the AI intent review aligned. Strict quantum correctness was not verified."
    : summary.decision === "inconclusive"
      ? "Verification unavailable — correctness has not been confirmed."
      : null;
  const action = advisoryOutcome
    ? "Treat this private artifact as AI-reviewed but unverified."
