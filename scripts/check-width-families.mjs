...
//     circuit it names.
//   * 245 browse rows. The number the roadmap promises, computed the way the
...
//     6, 6, 8, 8, 19, 4 and 6 more, so the number moved by exactly 69. Deliberately, which is what the error
...
// 176 at R2.6 (281 records), then +12, +6, +6, +8, +8, +19, +4 and +6 as the Zoo-
// and Classiq-parity intake batches added records that belong to no width family
// and therefore fold to nothing: 188, 194, 200, 208, 216, 235, 239, 245. See the
// header. The last +6 is the W22 Zoo-parity batch — matrix powers, string
// rewriting, zeta functions, Gauss sums, exponential congruences and subset
// finding — six literature records in no width family, so six new rows.
//
...
const EXPECTED_BROWSE_ROWS = 245;
