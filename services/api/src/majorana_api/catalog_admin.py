...
from .repos import artifacts, catalog, catalog_import, system
...
def orphaned_identities(
    published: Mapping[str, uuid.UUID], claimed: frozenset[str] | set[str]
) -> dict[str, uuid.UUID]:
    """Published records the manifest no longer claims.

    Separated from the query and the writes so the **rule** is testable without a
    database — the same argument `plan_re_attestation` makes, and the same stakes,
    because this set is what a `--authorize` run withdraws from the public catalog.

    Deliberately one-directional. An identity the manifest claims but which is not
    published is NOT an orphan and is not this function's business: that is either
    a record mid-import or one the attestation refused, and both are states the
    import path already reports. Treating it as symmetric difference would let a
    refused attestation look like something to delete.
    """
    return {
        identity: artifact_id
        for identity, artifact_id in published.items()
        if identity not in claimed
    }


async def _orphaned_identities(attested_by: uuid.UUID) -> tuple[dict[str, uuid.UUID], int]:
    """Published identities the manifest no longer claims, and the published total.

    Separated from both callers below because the reporting path inside
    `sync-bootstrap` and the acting path in `retire-bootstrap` must agree about
    what an orphan IS. Two implementations of that question would eventually
    disagree, and the direction that matters is the one where the actor retires
    something the reporter never showed anyone.
    """
    authority = CatalogAuthority.from_env()
    authority.require_configured()
    reviewer_scope = Scope(
    ...
    try:
        ...
        await engine.dispose()
    return orphaned_identities(published, claimed), len(published)


async def _retire_bootstrap(attested_by: uuid.UUID, authorize: int | None) -> None:
    """Withdraw published records the manifest no longer claims.

    ## Why this is not part of the import

    The importer has no delete branch, so deleting a record from the corpus
    changes the manifest and leaves the live row published. Every removal before
    this command existed was therefore a no-op against production, silently.

    ## Why it does not run unattended

    `sync-bootstrap` runs on every deploy. If this ran there too, one bad
    manifest generation — an entries file that fails to import, a filter that
    matches nothing — would withdraw the public catalog in a single deploy, and
    the deploy would report success while doing it. The manifest is generated by
    a script and checked by another script; neither is a reason to hand it an
    unattended delete.

    So the default is a report, and acting requires `--authorize N` where N is
    the exact number of orphans the operator has already looked at. Not a
    `--force`: a mismatch refuses without retiring anything, so an authorization
    written for one situation cannot execute a different one that arose in the
    meantime. That is the same shape as `--re-attest`, for the same reason.

    Soft delete, not a row delete: `deleted_at` is what the public predicate
    filters on, so the record stops being served and stays auditable and
    reversible. Restoring one is a matter of nulling that column.
    """
    authority = CatalogAuthority.from_env()
    authority.require_configured()
    reviewer_scope = Scope(
    ...
    orphans, published_total = await _orphaned_identities(attested_by)
    print(
        f"published={published_total} manifest_claims={len(BootstrapManifestSource().identities())} "
        f"orphaned={len(orphans)}"
    )
    for identity in sorted(orphans)[:100]:
        print(f"  orphan {identity}")
    if len(orphans) > 100:
        print(f"  … and {len(orphans) - 100} more")

    if not orphans:
        print("nothing to retire: every published record is claimed by the manifest")
        return
    if authorize is None:
        print(
            f"\nreport only. To withdraw these {len(orphans)}, re-run with "
            f"--authorize {len(orphans)}"
        )
        return
    if authorize != len(orphans):
        raise SystemExit(
            f"--authorize {authorize} does not match the {len(orphans)} orphans found. "
            "Refusing: the set moved since you looked at it. Re-run the report."
        )
    ...
    retired = 0
    try:
        for identity, artifact_id in sorted(orphans.items()):
            async with factory() as session:
                await artifacts.soft_delete_artifact(reviewer_scope, session, artifact_id)
                await session.commit()
            retired += 1
    finally:
        await engine.dispose()
    print(f"retire complete: withdrawn={retired}")
...
async def _sync_bootstrap(
    ...
    # Reports, never acts — see `_retire_bootstrap` for why an unattended delete
    # is the wrong shape here. Before this line a record removed from the corpus
    # was invisible in both directions: it stopped being imported and went on
    # being served, and no output anywhere said so. Now the deploy that ships the
    # removal is also the thing that tells you the removal has not taken effect.
    orphans, published_total = await _orphaned_identities(attested_by)
    if orphans:
        print(
            f"NOTE: {len(orphans)} of {published_total} published records are no longer "
            "claimed by the manifest and are still being served. They were deleted from "
            "the corpus; nothing withdraws them automatically. To withdraw them: "
            f"`catalog_admin retire-bootstrap --authorize {len(orphans)}`"
        )
        for identity in sorted(orphans)[:10]:
            print(f"  unclaimed {identity}")
        if len(orphans) > 10:
            print(f"  … and {len(orphans) - 10} more")


_NEEDS_REVIEWER = {"attest-bootstrap", "publish-bootstrap", "sync-bootstrap", "retire-bootstrap"}
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        metavar="N",
        help="retire-bootstrap only: withdraw the N published records the manifest no "
        "longer claims. N must equal the number the report found, exactly, or the run "
        "refuses without withdrawing anything. Without this flag retire-bootstrap only "
        "reports",
    )
    parser.add_argument(
        "--re-attest",
    ...
    async def _run() -> None:
        ...
        if args.command == "attest-bootstrap":
            ...
            await _retire_bootstrap(reviewer, args.authorize)
        else:
