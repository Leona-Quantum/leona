...
async def test_targeted_synthesis_preview_does_not_reserve_llm_tokens(scope, monkeypatch):
    async def token_reservation_must_not_run(*_args, **_kwargs):
        raise AssertionError("a code-free synthesis preview does not consume LLM tokens")
    ...
    request = runs.CreateRunRequest(
        task_prompt="Synthesize this Studio circuit",
        framework=Framework.QISKIT,
        mode=RunMode.EXECUTE,
        circuit_synthesis={
            "qubit_count": 1,
