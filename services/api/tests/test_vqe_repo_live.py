...
from majorana_api.orm import ArtifactVersion
...
async def test_workflow_components_create_list_and_reject_duplicate_role_ordinal(db):
    async with db() as session:
        scope = await _new_scope(session)
        workflow = await _make_artifact_version(scope, session, slug=f"workflow-{uuid.uuid4()}")
        ansatz = await _make_artifact_version(scope, session, slug=f"ansatz-{uuid.uuid4()}")
        await vqe.create_component_spec(
            scope,
            session,
            artifact_version_id=workflow.id,
            schema_version="0.1.0",
...
def _spec_kwargs(workflow_id: uuid.UUID, *, sha256: str, idempotency_key: str | None):
    return dict(
...
async def test_create_experiment_never_sets_run_id_and_is_idempotent(db):
    async with db() as session:
        scope = await _new_scope(session)
        workflow = await _make_artifact_version(scope, session, slug=f"exp-workflow-{uuid.uuid4()}")
        await session.commit()
    ...
    async with db() as session:
        first = await vqe.create_experiment(
            scope, session, **_spec_kwargs(workflow.id, sha256=sha, idempotency_key=key)
        )
        await session.commit()
    ...
    async with db() as session:
        ...
        with pytest.raises(vqe.IdempotencyConflictError):
            await vqe.create_experiment(
                scope,
                session,
                **_spec_kwargs(workflow.id, sha256="c" * 64, idempotency_key=key),
            )
...
async def test_experiment_is_invisible_outside_its_workspace(db):
    async with db() as session:
        owner_scope = await _new_scope(session)
        workflow = await _make_artifact_version(
            owner_scope, session, slug=f"exp-iso-{uuid.uuid4()}"
        )
        experiment = await vqe.create_experiment(
            owner_scope,
            session,
            **_spec_kwargs(workflow.id, sha256="d" * 64, idempotency_key=None),
        )
        intruder_scope = await _new_scope(session)
        await session.commit()
...
async def test_append_observation_enforces_all_its_invariants(db):
    async with db() as session:
        scope = await _new_scope(session)
        workflow = await _make_artifact_version(scope, session, slug=f"obs-{uuid.uuid4()}")
        experiment = await vqe.create_experiment(
            scope, session, **_spec_kwargs(workflow.id, sha256="e" * 64, idempotency_key=None)
        )
        await session.commit()
