...
def _reconcile_repetition_qec_contract(plan: Plan, task_prompt: str) -> Plan:
    ...
    updates: dict[str, Any] = {
        ...
        # The markers prove the request MENTIONS worst_case_fidelity, not that it
        # is the only requested output — every other requested key stays a durable
        # obligation, or check_contract stops requiring keys the user asked for.
        "expected_output_keys": list(
            dict.fromkeys(["worst_case_fidelity", *plan.expected_output_keys])
        ),
    }
...
# The wildcard gaps are bounded ({0,600}) because these run over the untrusted
# task prompt: an unbounded lazy `.*?` chain under DOTALL explores every split
# point on a long prompt that contains the first anchor but never completes the
# match — polynomial work per review attempt, attacker-controlled input. The
# fully-specified request family these match is compact by construction.
_AMPLITUDE_DAMPING_REQUEST = re.compile(
    rf"amplitude\s+damping\s+with\s+gamma\s*=\s*({_FINITE_NUMBER_PATTERN}).{{0,600}}?"
    rf"cos\(\s*({_FINITE_NUMBER_PATTERN})\s*\).{{0,600}}?"
    rf"exp\(\s*i\s*\*\s*({_FINITE_NUMBER_PATTERN})\s*\)\s*\*\s*"
    rf"sin\(\s*\2\s*\)",
    re.IGNORECASE | re.DOTALL,
)
_ONE_QUBIT_RY_RZ_REQUEST = re.compile(
    rf"\b(qibo|qulacs)\b.{{0,600}}?RY\(\s*({_FINITE_NUMBER_PATTERN})\s*\).{{0,600}}?"
    rf"RZ\(\s*({_FINITE_NUMBER_PATTERN})\s*\).{{0,600}}?in\s+that\s+order",
    re.IGNORECASE | re.DOTALL,
)
_FINITE_QPE_REQUEST = re.compile(
    rf"phase\s+estimation.{{0,600}}?eigenvalue\s+exp\(\s*2\s*\*\s*pi\s*\*\s*i\s*\*\s*"
    rf"({_FINITE_NUMBER_PATTERN})\s*\).{{0,600}}?use\s+(\d+)\s+counting\s+qubits",
    re.IGNORECASE | re.DOTALL,
)
...
def _amplitude_damping_reference_check(
    ...
    # A prompt match alone must not create obligations: if the user did not
    # request this profile's keys, demanding them fails candidates for omitting
    # output nobody asked for. Same gate on the other two prompt oracles.
    if expected is None or not set(expected).issubset(expected_output_keys):
        return None
    tolerance = 1e-8
    scores: dict[str, Any] = {}
...
def _one_qubit_rotation_reference_check(
    ...
    if match is None or not {"bloch_x", "bloch_y", "bloch_z", "probability_one"}.issubset(
...
def _finite_qpe_reference_check(
    ...
    for integer in range(dimension):
        ...
        # Closed-form Dirichlet kernel: |Σ_k e^{2πikδ}/N|² = sin²(πNδ)/(N²·sin²(πδ)),
        # exactly 1 at integer δ. The literal double sum was O(4^m) Python complex
        # arithmetic on the review path — tens of blocking seconds at m=12 inside
        # an async def with no await, repeated on every review attempt.
        denominator = math.sin(math.pi * delta) ** 2
        if denominator == 0.0:
            probabilities.append(1.0)
            continue
        numerator = math.sin(math.pi * dimension * delta) ** 2
        probabilities.append(numerator / (dimension**2 * denominator))
    dominant = max(range(dimension), key=probabilities.__getitem__)
    expected: dict[str, float | list[float]] = {
        "dominant_probability": probabilities[dominant],
        "phase_probabilities": probabilities,
    }
    runner_up = max((p for i, p in enumerate(probabilities) if i != dominant), default=0.0)
    if probabilities[dominant] - runner_up > 1e-8:
        # An exact tie makes either integer equally right, and the full
        # distribution comparison above already pins correctness — so the
        # argmax-derived scalars are demanded only when the peak is unique.
        expected["dominant_integer"] = float(dominant)
        expected["finite_phase_estimate"] = dominant / dimension
    return _request_numeric_reference_check(
        execution,
...
def _success_criteria_check(
    ...
    if lindblad_reference is not None:
        ...
        if (
            plan.verification_plan is not None
            and plan.verification_plan.reference_method == "independent_lindblad_extraction"
        ):
            # One unaudited model transcription is request-scoped evidence, not an
            # exact oracle. The renamed protocol stays out of
            # _EXACT_SUCCESS_PROTOCOLS, so a pass cannot be graded
            # VerificationMethod.EXACT / PHYSICAL — the grade dual-model
            # consensus earns.
            reference_details = reference_details | {
                "protocol": dict(reference_details["protocol"])
                | {
                    "name": "request_scoped_lindblad_evolution",
                    "reference_method": "independent_lindblad_extraction",
                },
            }
...
def _native_result_consistency_check(
    ...
    if payload is None:
        if not supports_native_result_consistency(plan.expected_output_keys):
            return None
        # The adapter records WHY collection failed (width past the ceiling, a
        # non-Qiskit FINAL_CIRCUIT, an observer error). A supported profile whose
        # trusted evidence is missing must surface as a recorded gap: silence here
        # made the check vanish on exactly the runs it was added for, with no
        # FAIL, no SKIPPED, and no record that nothing was compared.
        error = execution.observation.get("native_statevector_error")
        return {
            "method": "success_criteria",
            "result": "unavailable",
            "details": {
                ...
                "reason": (
                    "trusted native evidence for a supported RESULT profile was not collected"
                ),
...
def _reference_routing_critic(
    ...
    if protocols & {"exact_lindblad_evolution", "request_scoped_lindblad_evolution"}:
        instructions.append(
...
class ProductionSimplePipelinePorts:
    ...
    async def _reconcile_lindblad_reference(self, plan: Plan) -> SimplePortResult[Plan]:
        ...
        if truth_outside_range and comparison_references:
            # Only a dual-model-agreed oracle may erase the planner's declared
            # range. A single unaudited extraction that disagrees with the Plan is
            # exactly the case the REPLAN tripwire exists to expose — deleting the
            # range on its word would convert one model transcription into the
            # authority the range was there to check.
            criteria_updates["expected_range"] = None
        if plan.success_criteria.additional_notes is not None or "expected_range" in (
            criteria_updates
        ):
            updates["success_criteria"] = plan.success_criteria.model_copy(update=criteria_updates)
        return SimplePortResult.success(plan.model_copy(update=updates))
    ...
    async def review(
        ...
        if artifact_only:
            ...
            if native_consistency is not None:
                ...
                # An unusable or absent verifier is an evidence gap, not a
                # candidate defect: it stays visible above but must not be routed
                # into CODE_REPAIR/REPLAN, where it would spend the repair budget
                # on a verifier that could not apply its profile.
                if (native_consistency.get("details") or {}).get("fault") != "verifier_evidence":
                    reference_checks.append(native_consistency)
            amplitude_damping_check = _amplitude_damping_reference_check(
                self._task_prompt,
                execution,
                plan.plan.expected_output_keys,
            )
